...
void get_helmholtz_solution_Re(const Vector &x, double & p, double dp[],
                               double & d2p);
void get_helmholtz_solution_Im(const Vector &x, double & p, double dp[],
                               double & d2p);
...
int main(int argc, char *argv[])
{
   ...
   *nodes *= 1./domain_length;
   ...
   if (!scatter) // if scattering problem the source is zero and is driven by bc
   {
      ...
      if (!pml &&
          sol >=0) // if exact solution exists. Otherwise use homogeneous impedence (gradp . n + i omega p = 0)
      {
   ...
   ParComplexLinearForm bcopy(fespace,&b);
   bcopy.Assemble();

   ParSesquilinearForm acopy(fespace,&a);
   acopy.Assemble();

   OperatorHandle Ah;
   Vector X, B;
   // a.FormLinearSystem(ess_tdof_list, p_gf, b, Ah, X, B);
   // acopy.FormLinearSystem(ess_tdof_list, p_gf, bcopy, Ah, X, B);
   acopy.FormLinearSystem(ess_tdof_list, p_gf, bcopy, Ah, X, B);
   ...
   // acopy.RecoverFEMSolution(X,B,p_gf);
   acopy.RecoverFEMSolution(X,bcopy,p_gf);
   ...
   if (!scatter && sol >= 0 )
   {
      ...
      double loc_H1err_Re = p_gf.real().ComputeH1Error(&p_Re, &grad_Re, &one, 1.0,
                                                       h1_norm_type);
      double loc_H1err_Im = p_gf.imag().ComputeH1Error(&p_Im, &grad_Im, &one, 1.0,
                                                       h1_norm_type);
   ...
   if (visualization)
   {
      ...
      sol_sock_re << "solution\n" << *pmesh << p_gf.real() << keys << "valuerange\n" 
                  << -0.2 << ' ' << 0.2 <<
   ...
   if (pml)
   {
      ...
      for (int i = 0; i<dim; ++i)
      {
         if (x(i) >= c2)
         {
            ...
            dxs[i] = complex<double>(1.0,0.0) - complex<double>(0.0,coeff * pow(x(i)-lbeg,
                                                                                n-1.0));
         }
         if (x(i) <= c1)
         {
